...
- Modify: `tests/integration/dynamic_automation/test_qwen_mapping_smoke.py`
...
- [x] **Step 1: Write server-isolation and package-settings tests**
...
- [x] **Step 3: Add the package-owned settings model**
...
- [x] **Step 4: Remove dynamic behavior from shared settings**
...
- [x] **Step 5: Rewire only the Qwen evaluation path**
...
Update the conditional real-model smoke to construct
`DynamicAutomationSettings` as well. It is a dynamic-only consumer and must no
longer depend on the shared FastAPI settings object.
...
- [x] **Step 7: Run scoped regression and quality gates**

Run:

```text
python -m pytest tests/documents/dynamic_automation -o addopts='' -q
python -m pytest tests/integration/dynamic_automation/test_qwen_mapping_smoke.py -o addopts='' -q
...
- [x] **Step 8: Record the repository-wide baseline**
...
- [x] **Step 9: Commit the isolated implementation**

```text
git add app/core/config.py app/documents/dynamic_automation/config.py scripts/evaluate_dynamic_mapping.py tests/documents/dynamic_automation/test_mapping_config.py tests/integration/dynamic_automation/test_qwen_mapping_smoke.py docs/superpowers/plans/2026-08-12-dynamic-automation-config-isolation.md
git commit -m "fix(doc-automation): isolate mapping configuration"
```

The commit must contain only these six files.
