...
_TOKEN_PATTERNS: tuple[tuple[ProtectedTokenKind, re.Pattern[str]], ...] = (
    ...
    (
        ...
        re.compile(
            r"(?<!\w)(?:\d{4}-\d{2}-\d{2}|"
            r"\d{4}[./년]\s?\d{1,2}[./월]\s?\d{1,2}일?)(?!\w)"
        ),
    ),
    ("time", re.compile(r"(?<!\w)(?:\d{1,2}:\d{2}(?::\d{2})?|\d{1,2}시(?:\s?\d{1,2}분)?)")),
    ("amount", re.compile(r"(?<![\w,])\d{1,3}(?:,\d{3})+(?:\.\d+)?|(?<!\w)\d+\.\d+")),
    ("currency", re.compile(r"(?<![A-Za-z])(?:USD|KRW|EUR|JPY|GBP|CNY)(?![A-Za-z])")),
    ("unit", re.compile(r"(?<!\w)\d+(?:\.\d+)?(?:kg|g|mg|L|ml|cm|m)(?!\w)", re.IGNORECASE)),
    ("document_identifier", re.compile(r"(?<!\w)[A-Za-z]{2,}(?:-[A-Za-z0-9]+)+(?:\.\d+)?(?!\w)")),
    ("version", re.compile(r"(?<!\w)v\d+(?:\.\d+)+(?!\w)", re.IGNORECASE)),
    ("phone", re.compile(r"(?<!\w)\+?\d{1,3}(?:[- ]\d{1,4}){3,4}(?!\w)")),
